    ...
    public class UserController : ControllerBase
    {
        ...
        public async Task<string> GetIP()
        {
            ...
            foreach (IPAddress address in addresses)
            {
                ...
                {
                    // 创建一个匿名对象来存储 IP 地址
                    var ipInfo = new { IPAddress = address.ToString() };
                    // 使用 Newtonsoft.Json 进行序列化
                    string json = JsonConvert.SerializeObject(ipInfo);
                    return json;
                }
            }
            // 没有找到 IPv4 地址时的处理
        ...
        /// <returns></returns>
        [HttpGet]
        public async Task<List<SystemSetEntity>> GetSystemSet()
        {
            return await _setService.GetList();
        }
